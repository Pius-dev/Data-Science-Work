...
## Adding statistics : geom = c("points" , "smooth") this adds a 
...
### gggplot() function
## g+geom_point() this takes information from g objects and  produces scartterplot
## +geom_smooth() this adds low S mean curve with confidence interval
g <- ggplot(data, aes(var1, varl2)) # initiates call to ggplot amd specfies
...
cutPts<- quantile(df$cVar, seq(0,1,length=4), na.rm=TRUE) # this creats 
# quantile where the continuous variable will be cut.
