...
# first we load in the data we are to study in this case the copy of sales data 
...
# We load in ggplot2 package to help with the data visualization such that we can
# analyze the data.
library(ggplot2)
...
# For the next question we use a column graph to compare the sales of each 
# Rep and from the code below we can see it is Andrew.
S<-ggplot(Sales_data, aes(Rep, Total))
S + geom_col(fill = "limegreen") + labs(title = "REP SALES")
...
# We add up the total revenue from each of the tables for comparision
